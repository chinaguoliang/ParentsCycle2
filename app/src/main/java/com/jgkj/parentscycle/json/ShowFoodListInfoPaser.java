 ...
 */
public class ShowFoodListInfoPaser implements PaserJson {
    @Override
    public Object parseJSonObject(NetBeanSuper response) throws JSONException {
        Log.d("result", "the response code:" + response);
        if (response.obj != null) {
            ShowFoodListInfo tli = new ShowFoodListInfo();
            Type type = new TypeToken<List<ShowFoodListInfoItem>>() {}.getType();
            List<ShowFoodListInfoItem> atatol = GsonUtil.getListObject(response.getObj().toString(),type);
            tli.setDataList(atatol);
            response.setObj(tli);
            return response;
        } else {
            response.setObj(new ShowFoodListInfo());
            return response;
        }
    }
    ...
    public Object getErrorBeanData(String msg) {
        ...
        meData.setObj(new ShowFoodListInfo());
        return meData;
    }
