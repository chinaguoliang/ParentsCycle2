 ...
 */
public class GetSevenCowTokenPaser implements PaserJson {
    @Override
    public Object parseJSonObject(NetBeanSuper response) throws JSONException {
        if (response.obj == null) {
            response.setObj(new GetSevenCowTokenInfo());
        } else {
            GetSevenCowTokenInfo atatol=(GetSevenCowTokenInfo) JsonUtil.getObject(response.getObj().toString(), GetSevenCowTokenInfo.class);
    ...
    public Object getErrorBeanData(String msg) {
        ...
        meData.setObj(new GetSevenCowTokenInfo());
        return meData;
    }
