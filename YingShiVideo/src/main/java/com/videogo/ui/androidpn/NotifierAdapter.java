...
public class NotifierAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            // 获取list_item布局文件的视图
            convertView = mListContainer.inflate(com.videogo.open.R.layout.notifier_alarmloginfo_list_item, null);
            ...
            viewHolder.cameraItemBtn = (Button) convertView.findViewById(com.videogo.open.R.id.camera_item_btn);
            viewHolder.alarmName = (TextView) convertView.findViewById(com.videogo.open.R.id.name_tv);
            viewHolder.fromTv = (TextView) convertView.findViewById(com.videogo.open.R.id.from_tv);
            viewHolder.alarmTime = (TextView) convertView.findViewById(com.videogo.open.R.id.time_tv);
            viewHolder.messNumTv = (TextView) convertView.findViewById(com.videogo.open.R.id.message_num_tv);
            viewHolder.offlineIv = (ImageView) convertView.findViewById(com.videogo.open.R.id.item_offline);
            viewHolder.progressBar = (ProgressBar) convertView.findViewById(com.videogo.open.R.id.watting_pb);

            viewHolder.cameraItemRl = (LinearLayout) convertView.findViewById(com.videogo.open.R.id.camera_item_rl);

            viewHolder.cameraItemRlt = (RelativeLayout) convertView.findViewById(com.videogo.open.R.id.item_icon_area);
        ...
        viewHolder.iconIv.setImageResource(com.videogo.open.R.drawable.notify_bg);
        ...
        if (alarmLogInfo != null) {
            ...
            if (time.length() > 11) {
                ...
                if (alarmLogInfo.getNotifyType() == AlarmLogInfoEx.DEVICETYPE) {
                    if (time.substring(0, 10).trim().equals(getDate())) {
                        viewHolder.fromTv.setText(mContext.getString(com.videogo.open.R.string.today) + " " + time.substring(11));
                    } else {
                    ...
                    if (time.substring(0, 10).trim().equals(getDate())) {
                        viewHolder.alarmTime.setText(mContext.getString(com.videogo.open.R.string.today) + " " + time.substring(11));
                    } else {
            ...
            if (alarmLogInfo.getNotifyType() == AlarmLogInfoEx.DEVICETYPE) {
                ...
                viewHolder.cameraItemBtn.setBackgroundResource(com.videogo.open.R.drawable.cameralist_item_selector);
            ...
            if (alarmLogInfo.getNotifyType() == AlarmLogInfoEx.ALARMTYPE) {
                ...
                if (alarmType.hasCamera()) {
                    ...
                    DisplayImageOptions options = new DisplayImageOptions.Builder()
                            ...
                            .showImageForEmptyUri(com.videogo.open.R.drawable.notify_bg)
                            .showImageOnFail(com.videogo.open.R.drawable.event_list_fail_pic)
                ...
                viewHolder.progressBar.setVisibility(View.GONE);
                if (alarmLogInfo.getNotifyType() == AlarmLogInfoEx.DEVICETYPE) {
                    viewHolder.iconIv.setImageResource(com.videogo.open.R.drawable.device_offline);
    ...
    private void setAlarmName(ViewHolder viewHolder, final AlarmLogInfoEx alarmLogInfo) {
        ...
        if (alarmLogInfo.getNotifyType() == AlarmLogInfoEx.MESSAGETYPE) {
            ...
            if (alarmLogInfo.getAlarmType() == 2) {
                viewHolder.alarmName.setText(com.videogo.open.R.string.receice_leave_msg_video);
                // 1表示语音留言
            } else if (alarmLogInfo.getAlarmType() == 1) {
                viewHolder.alarmName.setText(com.videogo.open.R.string.receice_leave_msg_audio);
            } else {
                viewHolder.alarmName.setText(alarmLogInfo.getObjectName());
            }
            // 报警类型
        ...
        viewHolder.fromTv.setText(mContext.getString(com.videogo.open.R.string.push_event_from) + alarmLogInfo.getObjectName());
    }
